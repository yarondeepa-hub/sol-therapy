...
```
1. CLAUDE.md כבר נטען אוטומטית. אל תקרא אותו שוב. לפרטים מלאים ראה CLAUDE-full.md
...
4b. בדוק אם יש דוח Morning Scout שירון לא ראה:
   ...
   - אם יש דוח בלי .seen - הצג תדריך מובנה וצור קובץ .seen:
     a. תגלית מובילה: [שם + ציון + מה זה עושה - משפט אחד]
     b. הפתעה: [הפתעת הבוקר - משפט אחד]
     c. שולב אוטומטית: [מה נכנס ל-taste-profile, connected-tools, agent feeds]
     d. תור Lab: [פריטים שמחכים לבדיקה ידנית]
     e. ניסויים: [כמה ניסויים רצו, תוצאות בקצרה]
     f. בריאות שילוב: X/5 יעדים עודכנו (מתוך Integration Status בסוף הדוח)
   - אם בריאות שילוב < 3/5: הצג התראה "מנוע הלמידה לא שילב כראוי - בדוק"
4c. בדוק בריאות מנוע הלמידה:
   - קרא Learning Engine Feed ב-A-agents/illustrator-agent.md
   - אם Latest Discovery ריק או ישן מ-2+ ימים: התראה "מנוע הלמידה לא עדכן את סוכן האיור"
   - אותו דבר ל-A-agents/cto-agent.md
...
**איפוס סשן יזום (Proactive Session Reset):**

אחרי ABC-TOM של משימה כבדה, בדוק:
- האם הופעלו 3+ סוכנים במשימה?
- האם הסשן כולל 30+ הודעות?
- האם current-session.md מכיל 3+ סשנים?

אם אחד מהתנאים מתקיים - הצע לירון:
> "המשימה הושלמה. הסשן הזה ארוך - מומלץ לפתוח סשן חדש כדי לשמור על ביצועים. active-task.md מעודכן, אפשר להמשיך מיד בסשן הבא."

**חשוב:** לפני הצעת האיפוס, ודא ש:
1. active-task.md נקי או מכיל snapshot מדויק להמשכיות
2. current-session.md מעודכן עם כל מה שנעשה
3. ABC-TOM Loop נסגר (learning-log, feedback, decisions)
...
2. **שני מסלולים:** Team Sync מחליט - **Full Track** (מורכב, 2+ סוכנים) או **Fast Track** (נקודתי, סוכן יחיד, סיכון נמוך). ראה team-sync-agent.md לפירוט
3. **current-session.md תמיד מעודכן** - בכל שלב
4. **Handoff Template בין סוכנים** - חובה (Full Track)
5. **Gatekeeper Context Card** - חובה לפני בדיקה
6. **Gatekeeper תמיד לפני הצגה לירון** - לכל תוכן user-facing
7. **לולאת תיקונים אוטומטית** - הסוכנים מתקנים ביניהם, ירון רואה רק את הגרסה המאושרת
8. **"יוסי" = Full Track תמיד** - לא לדלג על שלבים כשירון פונה ליוסי
9. **מקביליות כברירת מחדל** - סוכנים שאין ביניהם תלות רצים במקביל. Team Sync מגדיר Parallel Groups בכל Intake
...
### דוחות וסיכומים לירון:
- **עברית בלבד. אפס מילים באנגלית.**
- שפה לא טכנית - כאילו מסביר לחבר, לא למתכנת
- אבל מנומקת - כל טענה עם סיבה, כל המלצה עם הסבר
- שמות כלים וטכנולוגיות - לתאר מה הם עושים במקום לזרוק שם באנגלית
- מספרים וציונים - כן. ז'רגון - לא.
- חל על: דוחות סקאוט, סקירה יומית, סיכומי משימות, כל מה שירון קורא
